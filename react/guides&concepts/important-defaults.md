...
在开箱即用的情况下，TanStack Query 配置了**激进但还算理智**的默认值。
有时，这些默认值有时会让新用户措手不及，或者说如果用户不知道它们，则会**使学习/调试变得困难**。
在继续学习和使用 TanStack Query 时，请记住以下几点：
...
如果某些重新获取的数据不是你所期望的，那么很大可能是你重新获取了窗口焦点(此时 React Query 正在处理[`refetchOnWindowFocus`](./window-focus-refetching.md))。
_在开发模式下，这可能会更频繁地触发_，特别是在浏览器的 Devtools 和你的应用之间互相切换也会导致获取，所以请额外注意。
...
- 默认情况下，"非活动"查询会在**5 分钟**后被垃圾收集。
  ...
  > 要更改这一点，可以将查询的默认`cacheTime`更改为`1000 * 60 * 5`毫秒以外的其他值。
